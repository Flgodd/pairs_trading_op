...
inline void slide(ArrayType& spread, const std::vector<double>& stock1_prices, const std::vector<double>& stock2_prices, double& sum, double& sq_sum) {
    if constexpr (I < N) {
        const double temp = stock1_prices[I] - stock2_prices[I];
        spread[I] = temp;
        sum += temp;
        sq_sum += (temp) * (temp) ;
        slide<I + 1, N>(spread, stock1_prices, stock2_prices, sum, sq_sum);
    }
}
...
void pairs_trading_strategy_optimized(const std::vector<double>& stock1_prices, const std::vector<double>& stock2_prices) {
    ...
    slide<0, N>(spread, stock1_prices, stock2_prices, sum, sq_sum);
